...
from models.scores_cond import (
    classtimecondTransformerScoreNet2,
    photometrycondTransformerScoreNet2,
    knobcondTransformerScoreNet,
)
...
################# a cross attn thingy #############
class simpleconditioningvdm(nn.Module):
    # base class for class and feature conditioning
    d_feature: int = 3
    timesteps: int = 1000
    gamma_min: float = -8.0
    ...
    def score_eval(self, flux, t, wavelength,spectime, conditioning, mask):
    ...
    def diffusion_loss(self, t, flux, wavelength, spectime,cond, mask):
    ...
    def __call__(self, flux, wavelength = None, spectime = None, conditioning=None,mask=None):
        ...
        loss_diff = self.diffusion_loss(t, flux_rec, wavelength, spectime,conditioning, mask)
    ...
    def sample_step(self, rng, i, T, z_t, wavelength = None, spectime = None, conditioning=None, mask=None):
...
#### class spectime conditioning 
class classtimecondVariationalDiffusionModel2(simpleconditioningvdm):
    ...
    def setup(self):
        ...
        if self.score == "transformer":
            self.score_model = classtimecondTransformerScoreNet2(
    ...
    @nn.compact
    def __call__(self, flux, wavelength = None, spectime = None, conditioning=None,mask=None):
        return super().__call__(flux, wavelength, spectime, conditioning, mask)   


#### class spectime conditioning 
class knobcondVariationalDiffusionModel2(simpleconditioningvdm):
    """Variational Diffusion Model (VDM), adapted from https://github.com/google-research/vdm
    ...
    def setup(self):
        ...
        # Score model specification
        if self.score == "transformer":
            self.score_model = knobcondTransformerScoreNet(
                d_t_embedding=self.d_t_embedding,
                score_dict=self.score_dict,
                num_classes=self.num_classes,
            )
...
class photometrycondVariationalDiffusionModel2(nn.Module):
    """Variational Diffusion Model (VDM), adapted from https://github.com/google-research/vdm
