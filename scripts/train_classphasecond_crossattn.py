...
np.save(f"Ia_samples_phase0_cross_attn.npy", samples.mean()[:, :, 0] * fluxes_std + fluxes_mean)
np.save(f"Ia_wavelength_phase0_cross_attn.npy", wavelength_cond[0]* wavelengths_std + wavelengths_mean)
...
plt.savefig(f'Ia_samples_phase0_cross_attn.png')  
plt.close()
...
with open(f'../ckpt/pretrain_classphasecond_static_dict_param_phase0_cross_attn', 'wb') as f:
    f.write(byte_output)
