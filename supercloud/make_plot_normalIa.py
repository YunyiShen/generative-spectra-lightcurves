...
normal_Ia = np.zeros(identities.shape)
#breakpoint()
total_normal_Ia = 0
for i in range(identities.shape[0]):
    for j in range(identities.shape[1]):
        params = re.findall(r'[-+]?\d*\.\d+e[-+]?\d+', identities[i,j])
        is_this_normal = True
        is_this_normal = is_this_normal and (np.abs(float(params[0])-1.) <= 0.3 )
        is_this_normal = is_this_normal and (np.abs(float(params[1])-1.4) <= 0.15 )
        is_this_normal = is_this_normal and (np.abs(float(params[2])-0.55) <= 0.1 )
        if is_this_normal:
            total_normal_Ia += 1
            normal_Ia[i,j] = 1
        else:
        ...
        # {kinetic energy [1e51 erg]}, {total mass [msun]}, {mass of nickel+Fe layer [msun]}, {mass of IME layer [msun]}, {mass of CO layer [msun]}, {int(10**m)}.h5
        # huristic normal Ia: fits around Nickel masses of ~0.5-0.6 solar mass. Explosion energy of ~ 10^51 ergs, mass of 1.4 Msol
print(total_normal_Ia/5)
normal_Ia = normal_Ia.astype(bool)
...
for i in range(5):
    mean_salt = np.nanmean(salt_test_loss[i, normal_Ia[i]], axis=0)
    sd_salt = np.nanstd(salt_test_loss[i, normal_Ia[i]], axis=0)
    if i == 0:
    ...
    #axes[0, 4-i].set_ylabel(f'   Phase {phase[i, normal_Ia[i]]}', labelpad=8, 
    ...
    mean_vdm = np.mean(vdm_test_loss[i, normal_Ia[i]], axis=0)
    sd_vdm = np.std(vdm_test_loss[i, normal_Ia[i]], axis=0)
    ...
    mean_salt_recon = np.nanmean(salt_recon_test_loss[i, normal_Ia[i]], axis=0)
    sd_salt_recon = np.nanstd(salt_recon_test_loss[i, normal_Ia[i]], axis=0)
    if i == 0:
    ...
    #mean_coverage = np.mean(vdm_coverage[i, normal_Ia[i]], axis=0)

    #mean_width = np.mean(vdm_width[i, normal_Ia[i]], axis=0)
    #sd_width = np.std(vdm_width[i, normal_Ia[i]], axis=0)
    all_mean_test_salt.append( float("{:.3g}".format( ( np.nanmean(salt_test_loss[i, normal_Ia[i]] ** 2)))))
    all_mean_test_vdm.append(float("{:.3g}".format(( np.nanmean(vdm_test_loss[i, normal_Ia[i]] ** 2)))))
    all_mean_test_salt_recon.append(float("{:.3g}".format(( np.nanmean(salt_recon_test_loss[i, normal_Ia[i]] ** 2)))))
...
for i in range(5):
    mean_coverage = np.nanmean(vdm_coverage[i, normal_Ia[i]], axis=0)
    mean_salt_coverage = np.nanmean(salt_coverage[i, normal_Ia[i]], axis=0)
    mean_salt_recon_coverage = np.nanmean(salt_recon_coverage[i, normal_Ia[i]], axis=0)
    ...
    #axes[0, i].set_ylabel(f'    Phase {phase[i, normal_Ia[i]]}', labelpad=8, 
    #                        rotation=90, ha='center')
    #axes[0, i].set_title(f'Phase {phase[i, normal_Ia[i]]}')
    axes[1, i].axhline(0.9, color='red', linestyle='--', linewidth=1.5)
    axes[1, i].tick_params(labelbottom=False)
    axes[1, i].set_ylim(0.01,1.05)


    mean_vdm_width = np.nanmean(vdm_width[i, normal_Ia[i]], axis=0)
    sd_vdm_width = np.nanstd(vdm_width[i, normal_Ia[i]], axis=0)

    mean_salt_width = np.nanmean(salt_width[i, normal_Ia[i]], axis=0)
    sd_salt_width = np.nanstd(salt_width[i, normal_Ia[i]], axis=0)

    mean_salt_recon_width = np.nanmean(salt_recon_width[i, normal_Ia[i]], axis=0)
    sd_salt_recon_width = np.nanstd(salt_recon_width[i, normal_Ia[i]], axis=0)
    ...
    all_mean_width.append(float("{:.3g}".format(np.nanmean(vdm_width[i, normal_Ia[i]]))))
...
plt.show()
plt.savefig(f'./plots/both{postfix}_normal_Ia.png')
